...
Track already stores a BYOK key. Full job-to-roadmap matching remains a later slice ([ADR-010](./010-deterministic-job-to-roadmap-matching.md)).
...
- **Rule-based extraction is the default** (CSS selectors on guest LinkedIn markup). Optional **Extract with AI** uses the Track BYOK model on sanitized HTML only, filling empty fields unless the user chooses replace.
