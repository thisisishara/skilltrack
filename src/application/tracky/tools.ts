...
import { explodeRoadmapDocument } from "@/domain/tracky/explode"
import { notePreview, type TrackyProposal } from "@/domain/tracky/proposals"
import { truncateJson } from "@/domain/tracky/context"
import {
  listChildSummaries,
  listRootSummaries,
...
function proposal(partial: Omit<TrackyProposal, "id" | "status">): TrackyProposal {
  return {
...
function proposalWithTopic(
  nodes: RoadmapNode[],
  partial: Omit<TrackyProposal, "id" | "status">,
  topicId: string | null
) {
  return proposal({
    ...partial,
...
export function createTrackyTools(input: {
  enabled: TrackyToolId[]
