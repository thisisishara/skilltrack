...
import {
  parseTrackDropRef,
  TRACK_REF_TEXT_PREFIX,
  upsertTrackRef,
  type TrackDropRef,
} from "@/domain/track/drop-ref"

const topic: TrackDropRef = {
  kind: "topic",
  id: "t1",
  title: "Python",
...
describe("parseTrackDropRef", () => {
  it("parses prefixed plaintext payloads", () => {
    expect(parseTrackDropRef(`${TRACK_REF_TEXT_PREFIX}${JSON.stringify(topic)}`)).toEqual(
      topic
    )
  })

  it("rejects random text", () => {
    expect(parseTrackDropRef("topic-1")).toBeNull()
  })
})
