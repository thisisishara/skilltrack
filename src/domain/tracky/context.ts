...
export type TrackyWorkingSet = {
  ...
  pendingProposals: Pick<TrackyProposalIndex, "id" | "kind" | "entity" | "title">[]
  scratchpad: string
  readBudget: {
    maxSteps: number
    policy: string
...
export function assembleWorkingSet(input: {
  ...
  return {
    ...
    pinned: input.pinned.slice(0, MAX_TRACKY_PINS),
