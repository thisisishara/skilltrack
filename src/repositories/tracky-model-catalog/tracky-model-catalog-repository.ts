...
export async function listTrackyModelCatalog(): Promise<TrackyExtraModels> {
  const supabase = getSupabaseServerClient()
  const extra = emptyExtraModels()
  const { data, error } = await supabase
    .from("tracky_model_catalog")
    .select("provider, model_id")
    .order("created_at", { ascending: true })

  if (error) {
    logEvent("error", "database.tracky_model_catalog.list_failed", {
...
export async function insertTrackyCatalogModel(
  provider: TrackyProvider,
  modelId: string
) {
  const supabase = getSupabaseServerClient()
  const { error } = await supabase.from("tracky_model_catalog").insert({
    provider,
    model_id: modelId,
  })
  ...
  if (error) {
    ...
    logEvent("error", "database.tracky_model_catalog.insert_failed")
...
export async function deleteTrackyCatalogModel(
  provider: TrackyProvider,
  modelId: string
) {
  const supabase = getSupabaseServerClient()
  ...
  if (error) {
    logEvent("error", "database.tracky_model_catalog.delete_failed")
